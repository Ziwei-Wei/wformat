...
from rd_format.utils import wheel_bin_path, wheel_data_path
...
class Uncrustify:
    ...
    def __init__(self):
        self.exe_path = wheel_bin_path("uncrustify")
        self.config_path = wheel_data_path("uncrustify.cfg")
    ...
    def update_config_with_docs(self) -> None:
        """
        ...
        try:
            res = subprocess.run(
                [self.exe_path, "-c", self.config_path, "--update-config-with-doc"],
                check=True,
                capture_output=True,
                text=True,
            )
            new_config_path = self.config_path.parent / "uncrustify.new.cfg"
            new_config_path.write_text(res.stdout, encoding="utf-8")
            new_config_path.replace(self.config_path)

        except Exception:
            print("[uncrustify] Failed to update config:")
