...
from rd_format.daemon import RdFormatDaemon
from rd_format.utils import *
...
def cli_app(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print("overview: Opinionated code formatter for C++ code in RdCore")
    print()
    ...
    if args.stdin:
        if sys.stdin.isatty():
            ...
            return 64  # EX_USAGE
        sys.exit(rd_format.run_stdin_pipeline())

    if args.serve:
        sys.exit(RdFormatDaemon(rd_format).serve())
    ...
    if len(file_paths) == 0:
        print("[Warning] No C++ file exist in the provided paths")
        return 0

    print(f"-- {len(file_paths)} C++ files exist")
    ...
    if args.check:
        if args.serial:
            retval = rd_format.check(file_paths)
        else:
            retval = rd_format.check_mt(file_paths)
    else:
        if args.serial:
            print("-- Using One Thread Only --")
            rd_format.format_inplace_many(file_paths)
        else:
            print("-- Using Multi-threading --")
            rd_format.format_inplace_many_mt(file_paths)

    if args.staged:
        print("-- Restaging files after formatting --")
        restage_files(file_paths)

    print(f"-- exit with code {retval}")
    return retval
