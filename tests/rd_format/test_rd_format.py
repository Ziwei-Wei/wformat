...
def test_required_samples():
    ...
    assert originals, "No original .cpp files found in required samples"
    ...
    if diffs:
        for d in diffs:
            print(d)
        pytest.fail(
            f"{len(diffs)} issue(s) found across {processed} file(s). See diff output above."
        )
...
def test_new_samples():
    ...
    assert originals, "No original .cpp files found in new samples"
